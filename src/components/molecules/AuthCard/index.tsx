    ...
    return (
      <Card style={{ width: CARD_WIDTH }}>
        <div className={styles.container}>
          ...
          <p>kazi {session?.user?.email}</p>
          <img src={session?.user?.image} alt={session?.user?.name} />

          <p className={styles.label}>Your email is: {session?.user?.email}</p>
          <p>{}</p>
          <Link
            type="primary"
      ...
      <Card style={{ width: CARD_WIDTH }}>
        <div className={styles.container}>
          <SkeletonButton
            active={true}
            block={true}
            style={{ marginBottom: 10, marginTop: 10, height: 42 }}
          />
          <SkeletonButton
            active={true}
            block={true}
            style={{ marginBottom: 10, height: 42 }}
          />
        </div>
      </Card>
    );
